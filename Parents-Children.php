...
<html lang="en" class="light">
  ...
  <style>
    .insert-button { background-color: cadetblue;
  color: white;
  border: none;
  padding: 4px 8px;
  border-radius: 4px;
  ...
  font-size: 16px; /* Increased font size */
        ...
        <div class="app-content-actions">
          ...
          <div class="app-content-actions-wrapper">
            ...
            <button class = "add-button" onclick="UserModal()">ADD</button>


            <div id="userModal" class="modal">
  <div class="modal-content">
  <img src="../TumagaHealthTrack/img/Barangaylogo.png" alt="A beautiful sunset over the mountains" width="120" height="100">
  <span class="close" id="closeModalE" onclick="closeUserModal()">&times;</span>
  <h2>PARENTS AND CHILDREN REGISTRATION</h2>
  <form id="parentChildren" action="http://localhost/TumagaHealthTrack/PhpFlow/Parent.php" method="post">
      ...
      <label for="patientMiddle">Middle Name</label>
      <input type="text" id="patientMiddle" name="patientMiddle" required>
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo '<th class="product-cell category">Lastname</th>';
    echo '<th class="product-cell status-cell">Time Added</th>';
    echo '<th class="product-cell sales">Contact#</th>';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $formattedTime = date("h:i A", strtotime($originalTime));
        ...
        echo '<td class="product-cell category">' . $row['firstname'] . '</td>';
        echo '<td class="product-cell status-cell">' . $row['lastname'] . '</td>';
        echo '<td class="product-cell status-cell">' . $formattedTime . '</td>';
        echo '<td class="product-cell status-cell">' . $row['contactno'] . '</td>';
      
     
        echo '<td class="product-cell price">';
        
        echo '<button class="insert-button" data-action="insert" data-id="' . $row['id'] . '" onclick="fetchData(this)">Add a Child</button>';
        echo '<button class="edit-button" data-action="view" data-id="' . $row['id'] . '">VIEW</button>';
...
<div id="ChildrenModal" class="modal">
  ...
  <div class="modal-content">
  <img src="../TumagaHealthTrack/img/Barangaylogo.png" alt="A beautiful sunset over the mountains" width="120" height="100">
  <span class="close" id="closeModalS">&times;</span>
  <h2>ADD A CHILD TO THE FAMILY</h2>
  <form id="parentChildren" action="http://localhost/TumagaHealthTrack/phpFlow/Child.php" method="post">
      ...
      <label for="childMid">Middle Name</label>
      <input type="text" id="childMid" name="childMid" required>
      <label for="childBirthdate">Date of Birth:</label>
  ...
  <input type="hidden" id="storedID" name="storedID" value="">
      
      &nbsp;
      <button type="submit" name="submit" id="submitChildrenButton">SUBMIT</button>
      <div class="success-msg" style="display: none;">
        <i class="fa fa-check"></i>
        Data has been sent successfully.
      </div>
...
// Get the insert button and add a click event listener to open the modal
document.addEventListener('DOMContentLoaded', function () {
  const insertButton = document.querySelector('.insert-button');

  insertButton.addEventListener('click', function () {
    openChildrenModal(); // Call the function to open the modal
  });
});
...
<script>
// Function to fetch parent ID
function fetchData(button) {
  let storedID;
  const id = button.getAttribute('data-id');

  if (id) {
    fetch('http://localhost/TumagaHealthTrack/PhpFlow/Fetch-Parent-Data.php?id=' + id)
      .then((response) => {
        if (response.ok) {
      ...
      .then((data) => {
        if (data && data.id) {
          const storedID = data.id; // Store the 'id' value in the global parentId variable
          console.log('Parent ID:', storedID);
          document.getElementById('storedID').value = storedID;
          submitChildren(storedID); // Call function to submit child data with parentId
        } else {
          console.error('No ID found in the received data.');
        }
      })
      .catch((error) => {
...
function submitChildren(storedID) {
  const submitChildrenButton = document.getElementById('submitChildrenButton');

  submitChildrenButton.addEventListener('click', function () {
    const childID = document.getElementById('childID').value;
    const childName = document.getElementById('childName').value;
    const childLastName = document.getElementById('childLastName').value;
    const childMid = document.getElementById('childMid').value;
  ...
  "childName": childName,
    ...
    fetch('http://localhost/TumagaHealthTrack/phpFlow/Child.php', {
      method: 'POST',
      body: JSON.stringify(childData), // Convert the object to a JSON string
      headers: {
...
const submitChildrenButton = document.getElementById('submitChildrenButton');
submitChildrenButton.addEventListener('click', function () {
  fetchData(submitChildrenButton);
});
...
<script>
// Function to open the modal and submit child data
function openChildrenModal() {
  const modal = document.getElementById('ChildrenModal');
  if (modal) {
    modal.style.display = 'block';
    ...
    const closeButton = document.getElementById('closeModalS');
...
// Function to fetch parent ID and open modal
document.addEventListener('DOMContentLoaded', function () {
  ...
  insertButtons.forEach(button => {
    ...
    <script src="../TumagaHealthTrack/Js/ajax-ParentC-form.js"></script>
    <script src="../TumagaHealthTrack/Js/script.js"></script>
    <script src="../TumagaHealthTrack/Js/ParentChildProf.js"></script>
